<script>
  ...
  $: giorni = siteConfig?.days?.filter((d) => d.enabled).map((d) => d.name) || [
    "Lunedì",
    "Martedì",
    "Mercoledì",
    "Giovedì",
  ...
  {:else if student}
    ...
    <div class="bg-[#252536] rounded-2xl p-6 mb-6 border border-gray-700/50">
      <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
        <div
          class="w-16 h-16 bg-[#FB773C]/20 rounded-full flex items-center justify-center flex-shrink-0"
        >
          <span class="text-2xl font-bold text-[#FB773C]"
            >{student.nomeCompleto.charAt(0).toUpperCase()}</span
          >
        </div>
        <div class="flex-grow">
          ...
          {#if student.classe}
            <span
              class="inline-flex items-center mt-2 bg-[#1e1e2e] text-gray-300 text-sm px-3 py-1 rounded-lg"
            >
        ...
        <div class="flex gap-2">
          {#if student.sdo}
            <span
              class="bg-blue-500/20 text-blue-400 text-xs font-medium px-3 py-1 rounded-lg"
              >SdO</span
            >
          {/if}
          {#if student.admin}
            <span
              class="bg-purple-500/20 text-purple-400 text-xs font-medium px-3 py-1 rounded-lg"
              >Admin</span
            >
          {/if}
        </div>
      ...
      <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-700/50 mt-4">
        <button
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-colors {student.admin
      ...
      <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-700/50 mt-4">
        {#if holesCount === 0}
          <div
            class="flex items-center gap-2 bg-green-500/10 text-green-400 px-4 py-2 rounded-xl"
          >
            <svg
              class="w-5 h-5"
              fill="none"
        ...
        {:else}
          <div
            class="flex items-center gap-2 bg-red-500/10 text-red-400 px-4 py-2 rounded-xl"
          >
            <svg
              class="w-5 h-5"
              fill="none"
            ...
            >
            {holesCount}
            {holesCount === 1 ? "buco" : "buchi"} nell'orario
          </div>
        {/if}
        <div
          class="flex items-center gap-2 bg-[#1e1e2e] text-gray-300 px-4 py-2 rounded-xl"
        >
          <svg
          ...
          >
          {corsi.length}
          {corsi.length === 1 ? "iscrizione" : "iscrizioni"}
        </div>
      </div>
    </div>
    ...
    {#if corsi.length === 0}
      <div
        class="bg-[#252536] rounded-2xl p-8 md:p-12 text-center border border-gray-700/50"
      >
        <div
          class="w-16 h-16 bg-[#FB773C]/20 rounded-2xl flex items-center justify-center mx-auto mb-4"
        >
          <svg
            class="w-8 h-8 text-[#FB773C]"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            ><path
    ...
    {:else}
      <div class="space-y-6">
        {#each enabledDays as day (day.id)}
          ...
          {@const holes = enabledHours
            .map((_, idx) => idx)
            .filter((h) => !coveredHours.has(h))}
          <div
            class="bg-[#252536] rounded-2xl border border-gray-700/50 overflow-hidden"
          >
            <div
              class="bg-[#FB773C]/10 border-b border-gray-700/50 px-5 py-3 flex items-center justify-between"
            >
              <h3 class="text-lg font-semibold text-[#FB773C]">{day.name}</h3>
              {#if holes.length > 0}
                <span class="text-xs text-red-400"
                  >{holes.length} {holes.length === 1 ? "buco" : "buchi"}</span
                >
              {/if}
            </div>
            <div class="p-4 space-y-3">
              {#each enabledHours as _, hourIdx}
                ...
                {#if corso}
                  <div
                    class="bg-[#1e1e2e] rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-4"
                  >
                    <div class="flex-shrink-0">
                      <span
                        class="inline-flex items-center justify-center min-w-16 h-10 px-3 bg-green-500/20 text-green-400 font-bold text-sm rounded-xl whitespace-nowrap"
                      >
                    ...
                    <div class="flex-grow min-w-0">
                      ...
                      <p class="text-gray-500 text-sm">
                        Aula: <span class="text-gray-300">{corso.aula}</span>
                      </p>
                    </div>
                    <div class="flex items-center gap-3 flex-shrink-0">
                      {#if corso.presente}
                        <span
                          class="inline-flex items-center gap-1.5 bg-green-500/20 text-green-400 text-xs font-medium px-3 py-1.5 rounded-lg"
                        >
                          <svg
                            class="w-4 h-4"
                            fill="none"
                      ...
                      {:else}
                        <span
                          class="inline-flex items-center gap-1.5 bg-gray-600/30 text-gray-400 text-xs font-medium px-3 py-1.5 rounded-lg"
                        >
                          <svg
                            class="w-4 h-4"
                            fill="none"
                ...
                {:else}
                  <!-- Hole -->
                  <div
                    class="bg-red-500/5 border border-red-500/20 border-dashed rounded-xl p-4 flex items-center gap-4"
                  >
                    <div class="flex-shrink-0">
                      <span
                        class="inline-flex items-center justify-center min-w-16 h-10 px-3 bg-red-500/10 text-red-400 font-bold text-sm rounded-xl whitespace-nowrap"
                      >
                        {getHourLabel(hourIdx)}
                      </span>
                    </div>
                    <div class="flex-grow">
                      <p class="text-red-400/70 text-sm italic">
                        Nessuna iscrizione
                      </p>
                    </div>
                  </div>
