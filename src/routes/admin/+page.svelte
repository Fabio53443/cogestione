<script>
    ...
    function showNote(name, text) {
        notePopup = { show: true, name, text };
    }
    function closeNote() {
        notePopup = { show: false, name: "", text: "" };
    }
    ...
    async function applyBulkNotes() {
        if (selectedStudents.length === 0) return;
        bulkNotesModal.loading = true;
        bulkNotesModal.error = null;
        try {
            const response = await fetch("/api/admin/studente/bulk-notes", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    studentIds: selectedStudents,
                    note: bulkNotesModal.note,
                    overwrite: bulkNotesModal.overwrite,
                }),
            });
            const data = await response.json();
            if (data.success) {
                bulkNotesModal.show = false;
                bulkNotesModal.note = "";
                bulkNotesModal.overwrite = false;
                selectedStudents = [];
                fetchData("students", pagination.page);
            } else {
                bulkNotesModal.error = data.message;
            }
        } catch (e) {
            bulkNotesModal.error = "Errore durante l'aggiornamento delle note.";
        }
        bulkNotesModal.loading = false;
    }
    ...
    onMount(() => {
        ...
        if (
            viewParam &&
            ["students", "courses", "teachers"].includes(viewParam)
        ) {
            fetchData(viewParam);
        }
    });
...
<div class="max-w-6xl mx-auto px-4">
            ...
            <div class="relative">
                ...
                <input
                    ...
                    placeholder={activeView === "students"
                        ? "Cerca studente per nome, email o classe..."
                        : activeView === "courses"
                          ? "Cerca corso per nome, descrizione o aula..."
                          : "Cerca organizzatore per nome o email..."}
    ...
    {:else if activeView && listData.length > 0}
        ...
        >
            ...
            {/if}
            {#if activeView === "students" && selectedStudents.length > 0}
                <div
                    class="p-4 border-b border-gray-700/50 bg-[#FB773C]/10 flex items-center justify-between"
                >
                    <span class="text-sm text-gray-300"
                        >{selectedStudents.length} studenti selezionati</span
                    >
                    <button
                        class="bg-[#FB773C] hover:bg-[#EB3678] text-white font-medium py-2 px-4 rounded-lg text-sm transition-colors flex items-center gap-2"
                        on:click={() => (bulkNotesModal.show = true)}
                    >
                        <svg
                            class="w-4 h-4"
                            fill="none"
                        ...
                        Aggiungi Note ({selectedStudents.length})
                    </button>
                </div>
            {/if}
                ...
                <table class="w-full">
                    <thead class="bg-[#1e1e2e] text-gray-400 text-sm">
                        <tr>
                            {#if activeView === "courses"}
                                <th class="px-4 py-3 text-left w-10">
                                    <input
                                        ...
                                        checked={selectedCourses.length ===
                                            listData.length &&
                                            listData.length > 0}
                                    />
                                </th>
                            ...
                            {/if}
                            <th
                                class="px-4 py-3 text-left cursor-pointer select-none hover:text-white transition-colors"
                                on:click={() => toggleSort("id")}
                            >
                                <div class="flex items-center gap-1">
                                    ID
                                    {#if sortColumn === "id"}
                                        <span class="text-[#FB773C]"
                                            >{sortDirection === "asc"
                                                ? "↑"
                                                : "↓"}</span
                                        >
                            ...
                            >
                                ...
                                <td class="px-4 py-3 text-white font-medium">
                                    {#if activeView === "students"}
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            {#if item.note}
                                                <button
                                                    class="text-yellow-400 hover:text-yellow-300 flex-shrink-0 transition-colors"
                                ...
                                {#if activeView === "students"}
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        {#if item.sdo}
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-500/20 text-purple-400"
                                                >SdO</span
                                            >
                                        {:else}
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        {#if item.holes > 0}
                                            <span
                                                class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold bg-red-500/20 text-red-400"
                                                >{item.holes}</span
                                            >
                                        {:else}
...
{#if resetPasswordModal.show}
    ...
    >
        <div
            class="bg-[#252536] rounded-2xl w-full max-w-md shadow-2xl border border-gray-700/50"
        >
            <!-- Header -->
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-gray-700/50"
            >
                <div>
                    <h2 class="text-lg font-bold text-white">Reset Password</h2>
                    <p class="text-sm text-gray-400 mt-0.5">
                        {resetPasswordModal.docenteName}
                    </p>
            ...
            <div class="px-6 py-5 space-y-4">
                {#if resetPasswordModal.success}
                    <!-- Success state -->
                    <div
                        class="bg-green-500/10 border border-green-500/30 text-green-400 px-4 py-3 rounded-xl text-sm"
                    >
                        Password aggiornata con successo!
                    </div>
                    {#if resetPasswordModal.generatedPassword}
                        <div
                            class="bg-[#1e1e2e] rounded-xl p-4 border border-gray-700/50 space-y-3"
                        >
                            <div>
                                <p class="text-gray-400 text-xs mb-2">Email:</p>
                                <div class="flex items-center gap-2">
                                    <code
                                        class="flex-1 text-white text-sm font-mono bg-[#252536] px-3 py-2 rounded-lg select-all"
                                    >
                                        {resetPasswordModal.docenteEmail}
                                    </code>
                                    <button
                                        class="p-2 bg-[#252536] rounded-lg transition-all duration-300 {copiedField ===
                                        'email'
                                            ? 'text-green-400'
                                            : 'text-gray-400 hover:text-white'}"
                                        title={copiedField === "email"
                                            ? "Copiato!"
                                            : "Copia"}
                                        on:click={() =>
                                            copyToClipboard(
                            ...
                            <div>
                                <p class="text-gray-400 text-xs mb-2">
                                    Nuova password (copiala ora, non verrà
                                    mostrata di nuovo):
                                </p>
                                <div class="flex items-center gap-2">
                                    <code
                                        class="flex-1 text-white text-lg font-mono bg-[#252536] px-3 py-2 rounded-lg select-all"
                                    >
...
{#if notePopup.show}
    ...
    >
        <div
            class="bg-[#252536] rounded-2xl w-full max-w-sm shadow-2xl border border-gray-700/50"
        >
            <div
                class="flex items-center justify-between px-5 py-3.5 border-b border-gray-700/50"
            >
                <div class="flex items-center gap-2">
                    <svg
...
<!-- Bulk Notes Modal -->
{#if bulkNotesModal.show}
    <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
    <div
        class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4"
        on:click|self={() => (bulkNotesModal.show = false)}
    >
        <div
            class="bg-[#252536] rounded-2xl w-full max-w-lg border border-gray-700/50 relative overflow-hidden"
        >
            <div
                class="flex items-center justify-between p-5 border-b border-gray-700/50"
            >
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-[#FB773C]/20 flex items-center justify-center text-[#FB773C]"
                    >
                        <svg
                            class="w-5 h-5"
                            fill="none"
                ...
                <button
                    class="text-gray-400 hover:text-white transition-colors"
                    on:click={() => (bulkNotesModal.show = false)}
                    aria-label="Chiudi"
                >
                    <svg
                        class="w-6 h-6"
            ...
            <div class="p-5 space-y-4">
                {#if bulkNotesModal.error}
                    <div
                        class="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 rounded-xl text-sm flex items-start gap-3"
                    >
                        <svg
                        ...
                        <span>{bulkNotesModal.error}</span>
                    </div>
                {/if}
                ...
                <div>
                    <label
                        class="block text-gray-400 text-sm font-medium mb-2"
                        for="bulk-note-text">Nota</label
                    >
                    <textarea
                        id="bulk-note-text"
                        bind:value={bulkNotesModal.note}
                        rows="4"
                        placeholder="Inserisci la nota che verrà applicata a tutti gli studenti selezionati..."
                        class="w-full bg-[#1e1e2e] border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#FB773C] focus:ring-1 focus:ring-[#FB773C] transition-colors resize-y"
                    ></textarea>
                </div>

                <div
                    class="flex items-center gap-3 bg-[#1e1e2e] p-3 rounded-xl border border-gray-700/50"
                >
                    <input
                        type="checkbox"
                        id="bulk-note-overwrite"
                        bind:checked={bulkNotesModal.overwrite}
                        class="w-4 h-4 rounded bg-transparent border-gray-600 text-[#FB773C] focus:ring-[#FB773C] focus:ring-offset-[#1e1e2e]"
                    />
                    <label
                        for="bulk-note-overwrite"
                        class="text-sm text-gray-300 cursor-pointer select-none"
                    >
                        <span class="font-medium text-white block"
                            >Sovrascrivi note esistenti</span
                        >
                        <span class="text-xs text-gray-500"
                            >Se non selezionato, la nuova nota verrà aggiunta in
                            fondo a quelle già presenti.</span
                        >
                    </label>
                </div>
                ...
                <div class="pt-2 flex gap-3">
                    <button
                        type="button"
                        class="flex-1 px-4 py-2.5 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white text-sm font-medium rounded-xl border border-gray-700 transition-colors"
                        on:click={() => (bulkNotesModal.show = false)}
                        disabled={bulkNotesModal.loading}
                    >
                        Annulla
                    </button>
                    <button
                        type="button"
                        class="flex-1 px-4 py-2.5 bg-[#FB773C] hover:bg-[#EB3678] text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        on:click={applyBulkNotes}
                        disabled={bulkNotesModal.loading ||
                            !bulkNotesModal.note.trim()}
                    >
                        {#if bulkNotesModal.loading}
                            <div
                                class="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin"
                            ></div>
                            <span>Salvataggio...</span>
                        {:else}
                            <span>Applica Note</span>
                        {/if}
                    </button>
                </div>
            </div>
