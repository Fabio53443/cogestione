    ...
    <div class="mb-8">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="relative" id="dropdown">
                ...
                {#if isDropdownOpen}
                    <div class="absolute z-10 mt-2 w-full sm:w-56 bg-[#252536] border border-gray-700 rounded-xl shadow-xl overflow-hidden">
                        {#each days as day, index}
                            ...
                            >
                                <div class="w-5 h-5 rounded border-2 flex items-center justify-center {selectedDays.has(index) ? 'bg-[#FB773C] border-[#FB773C]' : 'border-gray-600'}">
                                    {#if selectedDays.has(index)}
    ...
    {:else if filteredCorsi.length === 0}
        <div class="bg-[#252536] rounded-2xl p-8 text-center border border-gray-700/50">
            <p class="text-gray-400">
                {selectedDays.size === 0 ? 'Non ci sono corsi disponibili.' : 'Nessun corso per i giorni selezionati.'}
            </p>
        </div>
    {:else}
